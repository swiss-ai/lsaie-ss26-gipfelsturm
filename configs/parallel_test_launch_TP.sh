...
if [ -n "$WANDB_API_KEY" ]; then
    ...
    TRAINING_CMD="$TRAINING_CMD \
        ...
        --wandb-exp-name $EXP_NAME-parallel_test-$SLURM_JOB_ID"
else
...
SCRIPT="logs/${JOB_NAME}-parallel_test.sbatch"
...
#SBATCH --output=logs/%x-parallel_test-%j.log
#SBATCH --error=logs/%x-parallel_test-%j.log
#SBATCH --nodes=${NODES}
