...
if [ -n "$WANDB_API_KEY" ]; then
    ...
    TRAINING_CMD="$TRAINING_CMD \
        ...
        --wandb-exp-name $EXP_NAME-cuda_graph_te_attn-$SLURM_JOB_ID"
else
...
SCRIPT="logs/${JOB_NAME}-cuda_graph_te_attn.sbatch"
...
#SBATCH --output=logs/%x-cuda_graph_te_attn-%j.log
#SBATCH --error=logs/%x-cuda_graph_te_attn-%j.log
#SBATCH --nodes=${NODES}
