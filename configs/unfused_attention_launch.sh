...
if [ -n "$WANDB_API_KEY" ]; then
    ...
    TRAINING_CMD="$TRAINING_CMD \
        ...
        --wandb-exp-name $EXP_NAME-unfused_attention-$SLURM_JOB_ID"
else
...
SCRIPT="logs/${JOB_NAME}-unfused_attention.sbatch"
...
#SBATCH --output=logs/%x-unfused_attention-%j.log
#SBATCH --error=logs/%x-unfused_attention-%j.log
#SBATCH --nodes=${NODES}
